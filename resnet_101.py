...
from imagenet2clsid import id2cls
...
def resnet101_model(weights_path=None):
    ...
    x = conv_block(x, 3, [64, 64, 256], stage=2, block='a', strides=(1, 1)) #problem
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='b')
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='c')

    x = conv_block(x, 3, [128, 128, 512], stage=3, block='a')
    for i in range(1,4): #previously 3
...
if __name__ == '__main__':
  ...
  print(out.shape)
  print np.argmax(out)
  #print(model.__dict__.keys()) #in order to print all attributes
  print(id2cls(np.argmax(out)))
  new_out = np.sort(out, axis=None)
  #for top-k
  for i in range(1, 5):
    result = np.where(out == new_out[len(new_out)-i])
    print("\n ======= Top(%d) prediction ====\n"%i)
    print(result[1][0])
    print(id2cls(result[1][0]))
    print(out[0,result[1][0]])
